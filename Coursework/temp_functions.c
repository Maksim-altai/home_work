...
void month_print(uint16_t b, data* p)
{
    ...
    while (str_err[ernum] != 0)
    {
        if (b == 0)
        {
            fprintf(file_output,"  Error temp fail temp.csv str = %d \n", str_err[ernum]);
            return;
        }
        ernum++;
    }
    if (!num)
        fprintf(file_output, "  #   Year Month Nuvalue Ervalue MonthAvg MonthMax MonfMin \n");
    fprintf(file_output, " %2d  %5d %3d %6d %7d", num, p->year, p->mounth, b, ernum);
    int y = 0, minimum = p->temperature, maximum = p->temperature;
    for (uint8_t i = 0; i < b; i++)
    {
        y += p[i].temperature;
